...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = 'BE643E5D';
		$this->api_url = 'https://server.indiexd.com/';
		$this->api_key = '26BE1EC5BBD106E54F9A';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["c7d27e66a7de123"])){
				$_SESSION["c7d27e66a7de123"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["c7d27e66a7de123"])){
				$get_data = $this->call_api(
					'POST',
